...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment',  default='densenet201')
    ...
    val_transforms = transforms.Compose([
        lambda x: center_crop(x, 512),
        lambda x: rotate_90(x),
        lambda x: test_set_transform(x),
        lambda x: center_crop(x, int(args.crop_size)),
        transforms.ToTensor(),
        normTransform
    ])
    ...
    data_frame = pd.read_csv(os.path.join(args.dataset, 'train_data_external.csv'))
    ...
    data_frame_val = pd.read_csv(os.path.join(args.dataset, 'val_data.csv'))
    camera_id_val = data_frame_val['camera'].values
    fname_val = data_frame_val['fname'].values
    
    print(colored('==> ', 'blue')+'Initialized the loaders....')
    
    train_ds = CameraClassificationTrainDataset(args.dataset, fname_split=fname_train_shuffled, classes_split=camera_id_train_shuffled, transform=train_transforms, in_memory=args.in_memory)
    ...
    # Validation set
    val_ds = CameraClassificationTrainDataset(args.dataset, fname_split=fname_val, classes_split=camera_id_val, transform=val_transforms, mode='val_images', in_memory=args.in_memory)
    ...
    optimizer = optim.SGD(net.module.classifier.parameters(),lr=args.lr, weight_decay=args.wd)

    # Scheduler
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[args.unfreeze_epoch, 15, 30, 70, 120], gamma=0.1)
    # Criterion
    ...
    np.random.seed(args.seed)
    writer = SummaryWriter(os.path.join(args.logs, 'ieee_camera', 'new_validation', cur_snapshot))
    for epoch in range(args.n_epoch):
        ...
        if not os.path.isdir(os.path.join(args.snapshots, cur_snapshot, 'new_validation')):
            os.mkdir(os.path.join(args.snapshots, cur_snapshot, 'new_validation'))

        # Making logs backup
        np.save(os.path.join(args.snapshots, cur_snapshot, 'new_validation', 'logs.npy'), 
                [train_losses,val_losses,val_acc])

        if epoch > args.start_val:
            # We will be saving only the snapshot which has lowest loss value on the validation set
            cur_snapshot_name = os.path.join(args.snapshots, cur_snapshot, 'new_validation', 'epoch_{}.pth'.format(epoch+1))
